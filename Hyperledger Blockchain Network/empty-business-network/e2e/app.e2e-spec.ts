...
describe('Starting tests for empty-business-network', function() {
  ...
  it('website title should be empty-business-network', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('empty-business-network');
    })
  });

  it('network-name should be empty-business-network@0.0.2-deploy.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('empty-business-network@0.0.2-deploy.1.bna');
    });
  });

  it('navbar-brand should be empty-business-network',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('empty-business-network');
    });
  });
